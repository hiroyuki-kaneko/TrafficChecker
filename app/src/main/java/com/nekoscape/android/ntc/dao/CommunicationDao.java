...
public class CommunicationDao extends AbstractDao<Communication, Long> {
    ...
    public static final String TABLENAME = "CommunicationTable";

    /**
     * Properties of entity Communication.<br/>
    ...
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        ...
        db.execSQL("CREATE TABLE " + constraint + "'CommunicationTable' (" + //
                "'id' INTEGER PRIMARY KEY ," + // 0: id
                "'timestamp' text default (strftime('%s','now','localtime')) NOT NULL ," + // 1: timestamp
                "'years' text default (strftime('%Y','now','localtime'))," + // 2: years
                "'months' text default (strftime('%m','now','localtime'))," + // 3: months
                "'days' text default (strftime('%d','now','localtime'))," + // 4: days
                "'hours' text default (strftime('%H','now','localtime'))," + // 5: hours
                "'minutes' text default (strftime('%M','now','localtime'))," + // 6: minutes
                "'seconds' text default (strftime('%S','now','localtime'))," + // 7: seconds
                "'msend' long," + // 8: msend
                "'mrecv' long," + // 9: mrecv
                "'osend' long," + // 10: osend
                "'orecv' long);"); // 11: orecv
        // Add Indexes
        db.execSQL("CREATE INDEX " + constraint + "IDX_CommunicationTable_years ON CommunicationTable" +
                " (years);");
        db.execSQL("CREATE INDEX " + constraint + "IDX_CommunicationTable_months ON CommunicationTable" +
                " (months);");
        db.execSQL("CREATE INDEX " + constraint + "IDX_CommunicationTable_days ON CommunicationTable" +
                " (days);");
    }
    ...
    public static void dropTable(SQLiteDatabase db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "'CommunicationTable'";
        db.execSQL(sql);
    }
    ...
    protected void bindValues(SQLiteStatement stmt, Communication entity) {
        ...
        stmt.bindString(2, entity.getTimestamp());
    ...
    public Communication readEntity(Cursor cursor, int offset) {
        Communication entity = new Communication( //
                ...
                cursor.getString(offset + 1), // timestamp
    ...
    public void readEntity(Cursor cursor, Communication entity, int offset) {
        ...
        entity.setTimestamp(cursor.getString(offset + 1));
